...
OBSTACLE_GRID_TILE = pygame.image.load("trak2_wall3d.png")
...
class Game:
        ...
        for row in range(17):
            ...
            for col in range(33):
                if ((0 <= col <= 1) or (31 <= col <= 32)) and (6 <= row <= 9):
                    grid = Grid((grid_x, grid_y), SAFE_GRID_TILE, grid_size, is_safe_grid=True)
                    safe_grid.append(grid)
                    grid_group.append(grid)
                else:
                    grid_group.append(Grid((grid_x, grid_y), SPECIAL_TILE, grid_size))
    ...
    def make_map(self, grid_list):
        ...
        sprites.extend(wall_1.get_obstacle(grid_list[3][9], 36), OBSTACLE_GRID_TILE)
        sprites.extend(wall_3.get_obstacle(grid_list[10][9], 36), OBSTACLE_GRID_TILE)
        sprites.extend(wall_4.get_obstacle(grid_list[10][21], 36), OBSTACLE_GRID_TILE)
        sprites.extend(wall_2.get_obstacle(grid_list[3][21], 36), OBSTACLE_GRID_TILE)

        sprites.extend(wall_E.get_obstacle(grid_list[7][14], 36), OBSTACLE_GRID_TILE)
        sprites.extend(wall_E.get_obstacle(grid_list[6][17], 36), OBSTACLE_GRID_TILE)
